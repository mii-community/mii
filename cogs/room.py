...
class Room(Cog):
    ...
    async def open(self, ctx: Context):
        ...
        cat_room = self.get_cat_room()
        # ルームを持っていない場合
        if data_room is None:
            await self.create_new_room(creator, ctx)
            return
        # ルームカテゴリーにある場合
        ...
        if ch_room is None:
            await self.create_new_room(creator, ctx)
            return
        elif ch_room.category == cat_room:
    ...
    def get_cat_room(self):
        return self.bot.get_channel(constant.CAT_ROOM)

    async def create_new_room(self, author: discord.Member, sendto: discord.abc.Messageable):
        cat_room = self.get_cat_room()
        name = f"{author.display_name}の部屋"
        new_room = await cat_room.create_text_channel(name=name)
        await new_room.set_permissions(
            author, manage_messages=True, manage_channels=True
        )
        await sendto.send(f"{author.mention} {new_room.mention} を作成しました。")
